...
`IOCS_C_ASFL` でも変換できますが、負数の MZ 浮動小数点を渡した場合に overflow error になるケースを確認しています。たとえば `-12` 相当の `84,C0,00,00,...` は `IOCS_C_NMFL` では表示できましたが、`IOCS_C_ASFL` では overflow error になりました。
